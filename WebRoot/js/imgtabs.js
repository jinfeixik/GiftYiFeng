$.fn.extend({
	...
	nav_slide: function(options) {
		...
		function hideSlide() {
			t = setTimeout(function() {
				$(".query-box-nav").slideUp(100).css("z-index", 1);
				flag = false;
			}, 500);
		}
	},
	user_operate_slide: function(options) {
		...
		function hideSlide() {
			t = setTimeout(function() {
				$("#user_operate .user-box").fadeOut(300).css("z-index", 1);
				flag = false;
			}, 500);
		}
	...
	slider: function(settings) {
		...
		slider.prototype.showAni = function(index) {
			...
			if(this.params.animate_type == "slide") {
				this.params.slider_box.stop().animate({
					"left": this.params.arr_position[this.params.current]
				}, 500);
...
$.fn.countDown = function(times, callback) {
	var t = setInterval(decrement, 1000),
		_this = this;
...
$(function() {
	...
	$().nav_slide({
		eles: ["#gift_guide", "#flowers", "#cakes"]
	});
	$().user_operate_slide({
		eles: ["#nav_user", "#nav_cart"]
	});
