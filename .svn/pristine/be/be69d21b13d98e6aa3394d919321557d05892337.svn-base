	...
	<div id="app">
			...
			<div class="center">
				...
				<div class="operate fr m-t-31 rel" id="user_operate">
					...
					<div class="user-box user-signin dn" id="nav_user_box">
						...
						<dl class="tc">
						    <c:if test="${user==null}"><dt class="f-b28850"><a href="login.jsp">登录</a>&nbsp;<a href="register.jsp">注册</a></dt></c:if>
				...
				<div class="search fr m-t-31">
					<input type="text" name="search" id="message" ref="message" > <a
						href="javascript:void(0)" class="icon-bgr icon-search m-l-5" v-on:click="messageMethod"></a>
				</div>
			</div>
		</div>
		...
		<div class="currentloc bg-f5f5f5 clear">
			<div class="center f12">
				<div class="fl">
					<a href="index.jsp">首页</a><span>/</span> <a href="search.jsp">礼物搜索</a><span>/</span>
				</div>
				<a
					...
					<div class="query fl rel">
						...
						<dl class="clear">
							...
							<dd class="fl link-list">
								<template v-for="(item, index) in occation"> <a href="javascript:void(0);" @click="occationmethod($event)"
									id="occation"
		...
		<div class="cat-box">
			<div class="center">
				<div class="cat-title clear p-y-20">
					<div class="sorts fl">
								<template v-for="(item, index) in flag"> <a
									class="focus" v-if="index == 0">全部</a> <a
									href="javascript:void(0);" @click="flagmethod($event)"
									id="flag">{{item}}</a> </template>
					</div>
				</div>
				<ul class="cat-list clear">
					...
					<li class="fl p-30 m-t-20 ani-bd-shadow rel"
						...
						<p class="f12 f-666 tr m-t-20 clear com-diy">
							<span class="fr">销量：<strong>{{goods.sales }}</strong></span>
						</p>
					</li>
				...
				<div class="pages tc p-y-40">
					...
					<a class="prev icon-bgr icon-page-l" href="javascript:void(0);" @click="premethod($event)" ></a>
					<a class="next icon-bgr icon-page-r" href="javascript:void(0);" @click="nextmethod($event)"></a>
				</div>
			</div>
		</div>
			...
			console.log(totalPage)
				var vm = new Vue({
					...
					data: {
						...
						price:["100以下", "100-200", "200-300", "300-500", "500以上"],
						flag:["畅销", "最新", "价格由高到低", "价格由低到高"]
					},
					methods: {
						ajax:function(){
							$.ajax({
				  				...
				  				cache: false,
				  				type: "get",
				  				data:{message:message, occation:occation, price:price, object:object, classify:classify, flag:flag, curPage:curPage, totalPage:totalPage},
				  				dataType:"json",
				  				success: function(data){
					 				var jsonLength=Object.keys(data.search_list).length;
					  				...
					  				for(var i = 0 ; i < data.length ; i++){
					 			 		...
					 			 		totalPage = data.search_list[0].totalpage;
					 			 	}
					 			 	if(data.length == 0) {
					 			 		totalPage = 1;
					 			 	}
						...
						objectmethod: function(event) {
				           ...
				           el.parentElement.firstElementChild.className = '';
				        ...
				        pricemethod: function(event) {
				           ...
				           el.nextElementSibling.className = '';
				         	el.className='focus';
				           if(el.innerHTML=="100以下") {
				           	price=1;
				           } else if(el.innerHTML=="100-200") {
				           	price=2;
				           } else  if(el.innerHTML=="200-300") {
				           	price=3;
				           } else  if(el.innerHTML=="300-500") {
				           	price=4;
				           } else if(el.innerHTML=="500以上"){
				           	price=5;
				           }
				        ...
				        messageMethod:function() {
				        	...
				        	//alert("当前价格的内容："+message);
				        	vm.ajax();
				        },
				        flagmethod:function(event) {
				        	var el = event.currentTarget;
				           	flag=el.innerHTML;
				          	console.log("当前场合的内容："+el.innerHTML);
				           	el.className = 'focus';
