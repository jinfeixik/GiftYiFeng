...
function getCartInfo(){
	$.ajax({
	  url: "findShoppingCarInfo",
	  cache: false,
	  type: "POST",
	  dataType:"json",
	  success: function(data){
		  //var jsonObj = eval(data);
		  var jsonLength=Object.keys(data.goods_list).length;
		  data.length = jsonLength;
		  data.allAmount = 0;
		  data.totalNumber = 0;
		  //长度 总额 总数
		  for(var i = 0 ; i < data.length ; i++){
			  //console.log(data.goods_list[i].allAmount);
			  data.allAmount += Number(data.goods_list[i].amount);
			  data.totalNumber += Number(data.goods_list[i].num);
			  data.goods_list[i].id = Number(data.goods_list[i].id);
		  }
...
function updateCart(cart_info){
	...
	if(cart_info.totalNumber > 0){
		temp += '<table class="f12">';
		for(var i = 0 ; i < cart_info.length ; i++)
		{
			temp += '<tr class="gds"><td width="25%"><a href="'+'goods.php?id='+cart_info.goods_list[i].id+'"><img width="60px" src="'+cart_info.goods_list[i].image+'"></a></td>';
			temp += '<td class="overtxt" width="50%"><a href="'+'goods.php?id='+cart_info.goods_list[i].id+'">'+cart_info.goods_list[i].name+'</a><br>颜色样式：'+cart_info.goods_list[i].style+'</td>';
			temp += '<td class="tr" width="25%">X'+cart_info.goods_list[i].num+'<br>￥'+cart_info.goods_list[i].price+'<br><a class="remove_item" data-rec-id="'+cart_info.goods_list[i].id+'" href="javascript:void(0)">删除</a></td></tr>';
		}
		temp += '<tr><td colspan="3"><div class="clear m-t-10"><span class="fl">共计'+cart_info.totalNumber+'件商品<br><b>合计：<span class="f-d93732">￥'+cart_info.allAmount+'</span></b></span><a href="shopping_cart.jsp" class="fr btn-red-sml ani-bg">去购物车结算</a></div></td></tr></table>';
	}
	else{
	...
	if(cart_info.totalNumber > 0){
		$('#cart_count').html(cart_info.totalNumber).show();
	}else{
		$('#cart_count').html(cart_info.totalNumber).hide();
	}
