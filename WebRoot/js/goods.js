$(function() {
	...
	var $thumbslist = $("#goods_img_list").children("img"),
		$singlelist = $("#goods_single_list").children("img"),
		$loadbigimg = $(".load-big-img"),
		imgdefereds = [],
		$loading = $("#goods_single_list.loading"),
		currentindex = 0,
		length = 0,
		firstclick = true,
		previndex = 999;
	...
	function init_screen() {
		var $naturalpic = $("#naturalpic"),
			$tablist = $("#tablist");
		var dfd_screen = $.Deferred(),
			screendefereds = [];
	...
	function show_screen(currentindex) {
		if (currentindex != previndex) {
			var $naturalpic = $("#naturalpic img").not(".loading"),
				$tablist = $("#tablist li")
	...
	function loadGoodsTitle() {
		try {
			...
			var sizePos = $("#size").offset().top - 90,
				...
				guessyoulikePos = $("#guessyoulike").offset().top - 90,
				$service = $(".gds_service"),
				$qq_chat = $("#goods_qq_chat");
	...
	$('#add_to_cart').on('click', function() {
		...
		var goodStyleId = $("#goods_style_id").val();
		var price = new Number($("#price").val());
		console.log(goodStyleId);
		console.log(price);
		...
		$.ajax({
			  ...
			  cache: false,
			  type: "POST",
			  data:{goodStyleId:goodStyleId,price:price},
			  dataType:"json",
		...
		str  = str.substring(0, index+1) + "shopping_cart.jsp";
		console.log(str);
	...
	$("body").on("click", "#save_contact", function() {
		...
		if (!emailVal && !phoneVal) {
			...
			$.post('user.php?is_ajax=1&act=act_add_booking', {
				id: goods_id,
				number: 1,
				email: emailVal,
				tel: phoneVal
				...
				if (data.error == 0) {
					art.dialog.list["no_stock"].content(
						'<p style="padding-bottom:40px; width:805px; text-align:center">提交成功,感谢您对我们的信任和支持！</p>');
				} else {
	...
	function diy_pop(inputs) {
		...
		for (var i = 0; i < inputs.length; i++) {
			...
			if (inputs[i].max >= 20) {
				html += '<p><textarea class="value-input" type="text" name="diy_info[' + i +
					'][value]"></textarea><input type="hidden" value="' + inputs[i].title + '" name="diy_info[' + i +
					'][title]"/></p>';
			} else {
				html += '<p><input class="value-input" type="text" name="diy_info[' + i +
					'][value]"><input type="hidden" value="' + inputs[i].title + '" name="diy_info[' + i + '][title]"/></p>';
			}

		}
		html +=
			'<p class="tr m-t-31"><a href="javascript:void(0)" class="btn-red-single" id="buy_to_cart">加入购物车</a></p></div>';
		...
		var $diy_template = $("#diy_template"),
			$dialog = $(".aui_state_focus"),
			left = $diy_template.length > 0 ? $diy_template.offset().left : false,
			top = $diy_template.length > 0 ? $diy_template.offset().top : false,
			width = $diy_template.length > 0 ? $diy_template.width() : 0;
		if (left && top) {
			$("body,html").animate({
				scrollTop: top - 140
			}, 300, function() {
	...
	$("body").on("click", "#buy_to_cart", function() {
		...
		jQuery.post('flow.php?step=add_to_cart&check_diy=1&json=1', 'goods=' + encodeURIComponent(objToJSONString(goods)),
			function(data) {
				if (data.error > 0) {
					// 如果需要缺货登记，跳转
					if (data.error == 2) {
	...
	$('.select_attr').on('click', function() {
		...
		if (id == "select_attr0") { 
			var goods_style_id = $("#goods_style_id0").attr("vlaue");
			$("#goods_style_id").attr("value",goods_style_id);
			console.log("select_attr0");
			$('#goods_img_list0').prop('class', 'm-t-16 focus');
			$('#goods_single_list0').prop('style', 'display: inline;');
			$('#goods_img_list1').prop('class', 'm-t-16');
			$('#goods_single_list1').prop('style', 'display: none;');
		} 
		
		if (id == "select_attr1") { 
			var goods_style_id = $("#goods_style_id1").attr("vlaue");
			$("#goods_style_id").attr("value",goods_style_id);
			console.log("select_attr1");
			$('#goods_img_list1').prop('class', 'm-t-16 focus');
			$('#goods_single_list1').prop('style', 'display: inline;');
			$('#goods_img_list0').prop('class', 'm-t-16');
			$('#goods_single_list0').prop('style', 'display: none;');
		}
		...
		if ($('.img_id_' + img_id).length > 0) {
			...
			for (var i = 0; i < gallery.length; i++) {
				if (gallery[i].img_id == img_id) {
					...
					$('#goods_single_list img:visible').prop('src', gallery[i].bigpic).prop('data-cloudzoom', "zoomImage: '" +
						gallery[i].naturalpic + "'");
					break;
				}
			}
		}
...
function get_comments(page, goods_id, type, comment_cat, key) {
	...
	var url = 'comment.php?act=gotopage&page=' + page + '&id=' + goods_id + '&type=' + type + '&comment_cat=' +
		comment_cat + '&key=' + key;
...
function make_comment_list(data) {
	...
	var temp_html = '',
		lv_title = '';
	var temp_star = [
		'<div class="stars"><i class="icon-bgr icon-star "></i><i class="icon-bgr icon-star "></i><i class="icon-bgr icon-star "></i><i class="icon-bgr icon-star"></i><i class="icon-bgr icon-star"></i></div>',
		...
		if (data.comments.length > 0) {
			for (var id = 0; id < data.comments.length; id++) {
				...
				temp_html += '<li class="rel">' +
					...
					'<p class="lh2">' + data.comments[id].username + '&nbsp;&nbsp;<span class="f-808080">' + data.comments[id].add_time +
					'</span></p>' +
	...
	for (var i = 0; i < data.pager.pages.length; i++) {
		pager_html += '<a href="javascript:get_comments(' + data.pager.pages[i] + ',' + data.pager.goods_id + ',0,' + data.pager
			.comment_cat + ')" class="' + (data.pager.pages[i] == data.pager.page ? 'current' : '') + '">' + data.pager.pages[i] +
			'</a>';
	}
...
function make_question_list(data) {
	...
	if (in_goods_question_page) {
			...
			if (data.comments.length > 0) {
				for (var id = 0; id < data.comments.length; id++) {
					temp_html += '<li class="rel lh2 m-t-40">' +
						...
						'<p class="f-996c33">' + (data.comments[id].re_content ? 'A: ' + data.comments[id].re_content : '&nbsp;') +
						'</p>' +
						'</li>';
		...
		for (var i = 0; i < data.pager.pages.length; i++) {
			pager_html += '<a href="javascript:get_comments(' + data.pager.pages[i] + ',' + data.pager.goods_id + ',0,' + data.pager
				.comment_cat + ',\'' + data.key + '\')" class="' + (data.pager.pages[i] == data.pager.page ? 'current' : '') + '">' +
				data.pager.pages[i] + '</a>';
		}
		pager_html += '<a class="next icon-bgr icon-page-r" href="' + data.pager.page_next + '"></a>';
			...
			if (data.comments.length > 0) {
				for (var id = 0; id < data.comments.length; id++) {
					temp_html += (id % 2 != 0 ? '<li class="fl m-t-28 m-l-80">' : '<li class="fl m-t-28">') +
						'<p class="question clear"><span class="fl overtxt">Q: ' + data.comments[id].content +
						'</span><span class="fr f-9e9e9e dn">' + data.comments[id].add_date + '</span></p>' +
						(data.comments[id].re_content ? '<p class="f-996c33 overtxt">A: ' + data.comments[id].re_content + '</p>' :
...
function make_image_list(imgs) {
	...
	for (var i = 0; i < imgs.length; i++) {
		imghtml += '<span class="fl m-l-15 rel"><em><i class="icon-bgr icon-goods-zoom"></i></em><img src="' + imgs[i].src +
			'!list130"></span>';
		imgdata += (i == 0 ? '' : '+') + '{\'bigpic\':\'' + imgs[i].src + '!480\',\'smallpic\':\'' + imgs[i].src +
			'!goods80.jpg\'}'
	}
...
function initCommEvent() {
	...
	function initOrderPics(arr, index) {
		...
		var html =
			'<div class="goods-pics-layout rel"><img src="/themes/pc/images/ajax-loader.gif" class="loading" style=""><div class="pics-big rel">' +
