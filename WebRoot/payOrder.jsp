...
<meta name="Generator" content="www.liwuyou.com ">
		...
		<link type="text/css" rel="stylesheet" href="css/base.css?v20161225">
		<link type="text/css" rel="stylesheet" href="css/style.css?v20161225">
		<link type="text/css" rel="stylesheet" href="js/artDialog/.css">
...
<div class="header">
		...
		<div class="bg-f5f5f5 p-b-10">
			...
			<div class="currentloc bg-f5f5f5 clear">
				<div class="center f12">
					<div class="fl">
						<a href="index.jsp">首页</a><span>/</span>订单支付</div>
			...
			<div class="md-order center bg-fff">
				<div class="success-info clear b-b-3">
					<div class="up rel">
						<i class="icon-bgr icon-sucess"></i>

						<span class="order-code">订单号：${orderOrderNumber}</span>
						<p class="f30 f-78ac0a">订单提交成功</p>
						<p>请尽快完成支付，超时未支付订单将关闭。</p>
					</div>
					<div class="down rel b-t-1">
						<span class="total">应付总额：<input value="${allAmount}" style="display: none;"/><b class="f22 f-d93732">${allAmount}</b></span>
						<p class="lh2 f-666">收货信息：${tbUserAddressInfo.addressReciverName} ${tbUserAddressInfo.addressReciverPhoneNumber} ${tbUserAddressInfo.addressReciverAddress} ${tbUserAddressInfo.addressReciverDetailAddress} </p>
						<p class="lh2 f-666">
							<span>配送方式：
						    <c:choose>
				...
				<div class="success-info pay-box m-t-5">
					<h2 class="f22">请选择以下支付方式</h2>
					<h3 class="f16 m-t-40">支付平台</h3>
					<ul class="pay-platform clear m-t-20">
		...
		<input id="WIDout_trade_no" name="WIDout_trade_no"  value=${orderOrderNumber}/><br>
		<input id="WIDsubject" name="WIDsubject" value="yiyifangceshi" /><br>
		<input type="text"  id="WIDtotal_amount"  name="WIDtotal_amount" value=${allAmount}></br> 
		<input id="WIDbody" name="WIDbody" value="yiyifang"/><br>	
		</div>			
		<button  type="submit" style="border: 0;background-color: transparent;outline: none;"><li data-pay_id="4" title="支付宝"><i class="icon-pay icon-alipay"></i></li></button><br>
		</form>
        </div>
				</div>
			</div>
		...
		<input type="hidden" id="order_id" value="461881">
		<input type="hidden" id="pay_code" value="1">
		</div>
		...
		<script>
			$(function() {
				$('.pay-platform').on('click', '.fl', function() {
					$(".pay-platform .fl").removeClass('focus');
					$(this).addClass('focus');
					...
					$.ajax({
						...
						success: function(data) {
							if(data.success) {
								var paycode = $(self).data('paycode');
								if(paycode == 'wx_scan') {
									var img = $('#img_code').parent().html();
									var html = '<div class="wx-pay rel"><img src="/themes/pc/images/sao.png" class="sao"><p class="price f20">支付 ￥69.00</p>\
						<div class="tc ewm-box">\
						<p class="f24">微信扫码支付</p>\
						<p><span class="ewm dib b-1">' + img + '</span></p>\
						<p class="f14 lh2"><span class="f-d93732"><span id="timer">59</span>秒</span>后此二维码过期</p>\
						</div></div>';
									art.dialog({
										padding: 0,
										title: '',
									...
									$("#timer").countDown(300, function(flag) {
										if(flag) {
											$(arguments[2]).html(arguments[1]);
										} else {
											$(arguments[1]).parent().parent().html("二维码已失效");
										}
									});
								} else {
									var $pay_button = $(self).find('.pay_btn');
									//console.log($pay_button);
									if($pay_button.prop('tagName') == 'A') {
										var href = $pay_button.attr('href');
										window.open(href);
									} else {

										if($pay_button.attr('type') == 'submit') {
											$pay_button.parents('form').submit();
										} else if($pay_button.attr('type') == 'button') {
											eval($pay_button.attr('onclick'));
										}
									}
								}
							} else {
