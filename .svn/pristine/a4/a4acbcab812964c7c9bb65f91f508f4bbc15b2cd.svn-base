	...
	<body>
		...
		<div class="currentloc bg-f5f5f5 clear">
			<div class="center f12">
				<div class="fl">
					<a href="index.jsp">首页</a><span>/</span>我的订单</div>
			...
			<div class="center">
				...
				<div class="user-right fr bg-fff">
					...
					<div class="user-content">
						...
						<br>
                        <c:if test="${molist==null}">你还没有订单</c:if>
                        <c:if test="${molist!=null}">
                        
                        <c:forEach items="${myOrderAllList}" var="order">
					
						<ul class="order-list">
							<li class="m-b-20 b-1">	
						<form action=alipay.trade.page.pay.jsp method="post">
								<div class="top p-y-30 clear b-b-1 lh1">
								   <span class="fl-666 fl">
								       <input id="WIDout_trade_no" name="WIDout_trade_no"  value=${order.orderOrderNumber} style="display:none"/>
								                 订单号:${order.orderOrderNumber}
								   </span>
								   <span class="f-666 fl">
								       ${order.orderOrderTime}
								   </span>
								   <span class="f-666 fl">
								       <em class="f-d93732">总金额:${order.orderAmount}</em>
								       <input type="text"  id="WIDtotal_amount"  name="WIDtotal_amount" value=${order.orderAmount} style="display:none">
								      
								       <input id="WIDsubject" name="WIDsubject" value="yiyifangceshi" style="display:none"/>
								       <input id="WIDbody" name="WIDbody" value="yiyifang" style="display:none"/>
								   </span>
								   <span class="f-cdae7d fl tc">
								      <c:if test="${order.orderStatus==0}"><button  type="submit" style="border:0px;background-color:transparent; color:red">去支付</button></c:if><c:if test="${order.orderStatus==1}">已支付</c:if>								   
								   </span>
							    </div>
							    
								
						        <c:forEach items="${molist}" var="list">
						        <c:if test="${list.orderOrderNumber==order.orderOrderNumber}">
								  <div class="down">
									<p class="line clear rel"><span class="fl m-r-10"><img src="${list.goodsImagePath}"></span><span class="fl goods-name overtxt m-r-10">${list.goodsStyleName}</span><span class="fl">¥${list.goodsTypePrice}</span>
				
								    <a href="orderDetail?orderNumber=${order.orderOrderNumber}&styleId=${list.goodsStyleId}&imagePath=${list.goodsImagePath}" class="detail-btn dib tc">查看详情</a>
		                            </p>
		                         </div>
		                         </c:if>
		                         </c:forEach>
